...
NativeInteger_decode_ber(const asn_codec_ctx_t *opt_codec_ctx,
	 ...
	 */
	{
		...
		if((specs&&specs->field_unsigned)
			? asn_INTEGER2ulong(&tmp, (uint64_t *)&l) /* sic */
	...
	ASN_DEBUG("Took %"PRIi64"/%"PRIi64" bytes to encode %s (%"PRIi64")",
		(int64_t)rval.consumed, (int64_t)length, td->name, (int64_t)*native);
...
NativeInteger_encode_der(const asn_TYPE_descriptor_t *sd, const void *ptr,
    ...
    uint64_t native = *(const uint64_t *)ptr; /* Disable sign ext. */
...
NativeInteger_decode_xer(const asn_codec_ctx_t *opt_codec_ctx,
	...
	if(rval.code == RC_OK) {
		int64_t l;
		if((specs&&specs->field_unsigned)
			? asn_INTEGER2ulong(&st, (uint64_t *)&l) /* sic */
...
NativeInteger_encode_xer(const asn_TYPE_descriptor_t *td, const void *sptr,
	...
	asn_enc_rval_t er;
	const int64_t *native = (const int64_t *)sptr;
...
NativeInteger_decode_uper(const asn_codec_ctx_t *opt_codec_ctx,
	...
	if(rval.code == RC_OK) {
		if((specs&&specs->field_unsigned)
			...
			ASN_DEBUG("NativeInteger %s got value %"PRIi64"",
				td->name, *native);
	}
...
NativeInteger_encode_uper(const asn_TYPE_descriptor_t *td,
    ...
    native = *(const int64_t *)sptr;

    ASN_DEBUG("Encoding NativeInteger %s %"PRIi64" (UPER)", td->name, native);
...
NativeInteger_print(const asn_TYPE_descriptor_t *td, const void *sptr,
    ...
    if(native) {
        int64_t value = *native;
        ret = snprintf(scratch, sizeof(scratch),
                       (specs && specs->field_unsigned) ? "%"PRIu64"" : "%"PRIi64"", value);
...
NativeInteger_compare(const asn_TYPE_descriptor_t *td, const void *aptr, const void *bptr) {
    ...
    if(aptr && bptr) {
        const asn_INTEGER_specifics_t *specs =
            (const asn_INTEGER_specifics_t *)td->specifics;
        if(specs && specs->field_unsigned) {
            const uint64_t *a = aptr;
            const uint64_t *b = bptr;
            if(*a < *b) {
                return -1;
            } else if(*a > *b) {
                return 1;
            ...
            const int64_t *a = aptr;
            const int64_t *b = bptr;
            if(*a < *b) {
                return -1;
            } else if(*a > *b) {
                return 1;
