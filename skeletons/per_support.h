...
int uper_get_constrained_whole_number(asn_per_data_t *pd, uint64_t *v, int nbits);
 ...
 */
int per_long_range_rebase(int64_t v, int64_t lb, int64_t ub, uint64_t *output);
/* The inverse operation: restores the value by the offset and its bounds. */
int per_long_range_unrebase(uint64_t inp, int64_t lb, int64_t ub, int64_t *outp);

/* X.691-2008/11, #11.5 */
int uper_put_constrained_whole_number_u(asn_per_outp_t *po, uint64_t v, int nbits);
