 ...
 * using the generic 'int64_t' or 'uint64_t' type.
 */
...
/*
 * Since we don't know the sizeof(int64_t) on the possible target platform
 * which will be compiling the code generated by asn1c, let's play it
 * simple: int64_t's range is equal to or greater than int32_t.
...
#define	LEFTMIN		(-RIGHTMAX-1)	/* of 32-bit integer type */
	...
	/* The "fits into int64_t" operation is relevant only for integer types */
	switch(expr->expr_type) {
	case ASN_BASIC_INTEGER:
	case ASN_BASIC_ENUMERATED:
		break;
	...
	if(range) {
		if(!range->incompatible) {
			right = range->right;
			/* Use 4 instead of sizeof(int64_t) is justified! */
